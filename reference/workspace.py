...
for i in range(1, 301, 2):
    for j in range(1, 301, 2):
        for k in range(1, 301, 2):
            [cosb,cosc,cosd] = Delta_Inversesolution2(k,j,i) # You need to define this function
            [x1,y1,z1] = Delta_Inversesolution(k,j,i) # You need to define this function
            if (abs(cosb)<0.34 and abs(cosc)<0.34 and abs(cosd)<0.34 and z1>-430):
                m += 1
                [x[m],y[m],z[m]] = Delta_Inversesolution(k,j,i) # You need to define this function

# 绘制工作空间
fig = plt.figure()
ax = fig.add_subplot(111, projection='3d')
scatter = ax.scatter3D(x, y, z, c=x, cmap='jet')
plt.colorbar(scatter)
ax.set_xlabel('X')
ax.set_ylabel('Y')
ax.set_zlabel('Z')
plt.title('Delta并联机器人工作空间')
plt.axis('square')
plt.show()
